...
class m_integrated_service extends MY_Model
{
    ...
    public function select_numbers_model($ins)
    {
        ...
        $this -> db -> where('classification',2);
        $res = $this->db->get();
...
//|----------------------------------------------------修改------------------------------------------------------------|
    ...
    {
        $res = $this->db->select("$project")->where('score_company', $unit_num)->get('integrated_service')->row_array();
        return $res;
    }
    ...
    {
        $res = $this->db->select("$project")->where('score_company', $unit_num)->get('integrated_service')->result_array();
        $data[$project] = $res[0][$project] - $score;
        $info = $this->db->where('score_company', $unit_num)->update("integrated_service", $data);
        return $info;
    }
    ...
    {
        $data = $this->db->select('score_month')->get('integrated_service')->result_array();
    ...
    public function add_data($score)
    {
        ...
        foreach ($data as $k => $v) {
            ...
            $res = $this->db->insert('integrated_service', $score);
        }
        return $res;
    }
