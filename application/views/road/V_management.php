            ...
            <div>
                <form method="post" style="float: left">
                    ...
                    <select  id="bmmc"  style="height: 26px;width: 200px" name="bmmc">
                        ...
                        <?php foreach ($bmmc as $v){?>
                            <option value="<?php echo $v['orgnum'];?>" <?php if($orgnum == $v['orgnum']){ echo 'selected';}else{ echo '';}?> ><?php echo $v['orgname'];?></option>
                        <?php }?>
                    </select>
...
<script>
    ...
    $(".export").click(function(){
        ...
        layer.confirm('是否导出该表？', {
            ...
            window.location.href = '<?php echo base_url();?>admin/c_management/export?czr='+czr+'&&bmmc='+bmmc+'&&startTime='+startTime+'&&endTime='+endTime;
            layer.closeAll();
        }, function(){
            layer.closeAll();
        });
...
<script>
    $('#search').click(function (){
        ...
        $.ajax({
            url:'<?php echo base_url()?>admin/c_management/search',
            type:'post',
            data:{'czr':czr,'bmmc':bmmc,'startTime':startTime,'endTime':endTime},
            success:function(data)
            {
               ...
               for(var i=0;i<data1['data'].length;i++){
                   var paiming = i+1;
                    ...
                    str += "<th>"+data1['data'][i]['realname']+"</th>";
                    str += "<th>"+data1['data'][i]['accounts']+"</th>";
