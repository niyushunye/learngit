        ...
        <div class="tab-panel active" id="tab-set" style="">
            <form method="post" class="form-x">
                <div class="form-group">
                    ...
                    <div class="field">
                        <input type="text" class="input title"  value="<?php echo $shuju['title']?>" id="title ">
                    </div>
                </div>
                <div class="form-group">
                    <div class="label" style="color:#0C0C0C">
                    ...
                    <div class="field">
                        <div id="summernote"><?php echo $shuju['content']?></div>
                    </div>
                </div>
                <input type="hidden" value="<?php echo $shuju['id']?>" class="id">
                <div class="form-group">
                    <div class="label" style="color:#0C0C0C">
                    ...
                    <div class="field">
                        <?php foreach ($data as $k => $v){?>
                            ...
                            <input type="checkbox" name="vehicle" ass="1" class="fuxuan" <?php foreach ($orginfo as $k6 => $v6){if($v6 == $v['orgnum']){ echo 'checked';}}?> value="<?php echo $v['orgnum']?>"> &nbsp;<span class="dianji" srr="<?php echo $v['orgnum']?>"><?php echo $v['orgname']?></span><br>
                            <div class="jwry<?php echo $v['orgnum']?> add">
                                <?php foreach ($v['jwry'] as $k2 => $v2){?>
                                    <label style="padding-left: 20px">
                                        <input type="checkbox" class="i-checks vote<?php echo $v['orgnum']?> vote" <?php foreach ($jwry as $k3 => $v3){if($v3 == $v2['accounts']){ echo 'checked';}}?> value="<?php echo $v2['accounts']?>"  name="receive_account[]">
                ...
                <div class="form-group">
                    ...
                    <div class="field">
                        <select class="input work_type" id="work_type ">
                            <option value="1" <?php if($shuju['work_type'] == 1){echo  'selected';}?>>日常</option>
                            <option value="2" <?php if($shuju['work_type'] == 2){echo  'selected';}?>>党建</option>
                            <option value="3" <?php if($shuju['work_type'] == 3){echo  'selected';}?>>法制</option>
                            <option value="4" <?php if($shuju['work_type'] == 4){echo  'selected';}?>>宣传</option>
                            <option value="5" <?php if($shuju['work_type'] == 5){echo  'selected';}?>>指挥中心</option>
                            <option value="99" <?php if($shuju['work_type'] == 99){echo  'selected';}?>>其他</option>
                        </select>
                    </div>
                </div>
...
<script type="text/javascript">
    $('.operate_sub').click(function (){
        ...
        if(title == ''){
            ...
            $.ajax({
                url:'<?php echo base_url()?>admin/c_issued/edit_save',
                type:'post',
                data:{ 'id':id,'title':title,'content':content,'task_orginfo':task_orginfo,'work_type':work_type,'vote':vote},
                success:function(data)
                {
                    if(data == '1')
                    {
...
<script>
    ...
    $('.fuxuan').change(function (){
        var a = $(this).val();
        //var b = $(this).attr('ass');
        var aaa = $(this).is(':checked');
        $('.vote'+a).each(function(){
            $(this).prop('checked',aaa);
        })
    });
</script>
</body>
