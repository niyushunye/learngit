...
class c_non_vehicle extends CI_Controller{
...
//                'isParent' => true
            //点击时指向读取功能
            'url' => base_url().'admin/c_non_vehicle/filelist/'.$data['quhua'][0]['number'],
            'target' => "iframe1",
            "open" =>"true"
        );
        $data['result'][] = $arr;
...
//                    'isParent' => true
                    //点击时指向读取功能
                    'url' => base_url().'admin/c_non_vehicle/filelist/'.$value1['number'],
                    'target' => "iframe1"
                );
                $data['result'][] = $arr1;
            }
...
//            echo $orgnum;
            ...
            $this->load->view("non_vehicle/V_index",$data);
        }
//        print_r($data);exit();
    }
...
//        echo $orgnum;
        ...
        if($orgnum > 1000){
            ...
            $config['first_url'] =  base_url() . '/admin/c_non_vehicle/filelist/0/'."/"."$orgnum";
        }
        $orgnums = $this -> m_non_vehicle -> orgnums($orgnum);
        foreach ($orgnums as $k => $v){
            foreach ($v as $k1 => $v1){
                $orgnumss[] = $v1;
            }
        }
        $data['memberinfo'] = $this -> m_non_vehicle -> select_vehicle($orgnumss,$curpage,$num);
        $data['total'] = $this -> m_non_vehicle -> select_vehicle_count($orgnumss,$curpage,$num);
        $config['per_page'] = $num;
        $config['base_url'] = base_url().'/admin/c_non_vehicle/filelist/';
        $config['suffix'] = "/"."$orgnum" ;                                                      //给链接地址加后缀  http://localhost/ci/控制器/方法名/参数
        $config['total_rows'] = $data['total'];
        $this->pagination->initialize($config);
        $this->load->view('public/header');
        $this->load->view("non_vehicle/V_index_non",$data);
    }

    //查看详情
    public function details(){
        $id = $this -> input -> get('id',TRUE);
        $data = $this -> m_non_vehicle -> select_details($id);
        $this->load->view('public/header');
        $this -> load -> view('non_vehicle/V_index_nos',$data);
    }
    ...
    public function delete(){
        ...
        $info = $this -> m_non_vehicle -> delete_select($id);
        if($info){
            echo 1;    //删除成功
        }else{
            echo 2;    //删除失败
    ...
    public function edit(){
        ...
        $data = $this -> m_non_vehicle -> select_details($id);
        $this -> load ->view('public/header');
        $this -> load -> view('non_vehicle/V_index_edit',$data);
    }
    ...
    public function edit_up(){
        $id = $this -> input -> post('id',TRUE);
        $data['hphm'] = $this -> input -> post('hphm',TRUE);
        $data['cjh'] = $this -> input -> post('cjh',TRUE);
        $data['ppxh'] = $this -> input -> post('ppxh',TRUE);
        ...
        $res = $this -> m_non_vehicle -> select_upload($id,$data);
        if($res){
            echo 1;  //修改成功
        }else{
            echo 2;  //修改失败
    ...
    public function sousuo(){
        ...
        $data['memberinfo'] = $this-> m_non_vehicle -> where_select_sousuo($clsyr,$sfzh,$xzqh);
        $data['total'] = $this -> m_non_vehicle -> where_select_count($clsyr,$sfzh,$xzqh);
        $data['sfzh'] = $sfzh;
        $data['orgnum'] = $xzqh;
        $data['clsyr'] = $clsyr;
        $this->load->view('public/header');
    ...
    public function export(){
        ...
        //标题
        $objPHPExcel->getProperties()->setTitle("无牌机动车统计表");
        //题目
        $objPHPExcel->getProperties()->setSubject("无牌机动车统计表");
        //描述
        $objPHPExcel->getProperties()->setDescription("无牌机动车统计表");
        //关键字
        $objPHPExcel->getProperties()->setKeywords("无牌机动车统计表");
        //种类
        $objPHPExcel->getProperties()->setCategory("无牌机动车报表汇总");

        //设置当前的sheet
        $objPHPExcel->setActiveSheetIndex(0);
        //设置sheet的name
        $objPHPExcel->getActiveSheet()->setTitle('无牌机动车统计');
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('A1', '汉滨区无牌机动车统计');
        ...
        //获取数据
        $result_array = $this -> m_non_vehicle -> select_tj();
